...
启动器把后端和前端的 PID、Linux process start time 与标签写入按工作区隔离的临时 PID 文件。
start time 从 `/proc/<pid>/stat` 的括号进程名之后解析，因此 Next.js 把进程名改成包含空格的文本也不会
造成字段错位。若主脚本被 `SIGKILL`、终端崩溃或其他不可捕获异常中止，下次 `make dev` 会先校验
start time，递归终止匹配的旧进程树并等待退出；超时才强制结束。随后通过当前 cluster 的 `pg_ctl`
停止残留 PostgreSQL，再依序重启数据库、后端和前端。start time 不匹配时视为 PID 已复用，不操作
该进程，避免终止无关程序。
...
- 主脚本被 `SIGKILL` 且数据库、Go、Next.js 均残留时，下次启动能清理旧进程并成功占用三个原端口。
- 前后端生产构建通过，容器健康检查正常。
