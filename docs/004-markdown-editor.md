...
页面加载时同时读取两个方向，使页头入口无需打开面板即可展示完整 Unique 数量；成功结果缓存 60 秒。
缓存未过期时重复开关不发请求；缓存过期后再次打开面板会后台刷新。正文保存产生新的
`serverRevision` 时，无论面板是否打开都刷新关系和数量；如果首次加载仍在途，则取消旧请求并按新
revision 重发，避免保存前快照覆盖最新结果。刷新失败继续展示旧数据和 Retry，不影响编辑或保存。
...
`>=1024px` 时，页头在视图模式控件之后、Outline/Details 之前提供唯一的链形图标入口。关系请求完成前
