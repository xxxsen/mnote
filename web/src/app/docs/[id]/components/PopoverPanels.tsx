      ...
      {activePopover === "color" && renderPopover(
        <div className="p-3 bg-background border border-border rounded-lg shadow-xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-4 gap-2 w-48">
            {COLORS.map((c) => (
              <button type="button" key={c.value || "default"} aria-label={c.label} onClick={() => handleColor(c.value)} className="h-8 w-full rounded-lg border border-input hover:scale-105 transition-transform flex items-center justify-center" style={{ backgroundColor: c.value || "transparent" }} title={c.label}>
      ...
      {activePopover === "size" && renderPopover(
        <div className="p-3 bg-background border border-border rounded-lg shadow-xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex flex-col gap-1 w-32">
            {SIZES.map((s) => (
              <button type="button" key={s.value} aria-label={`Set font size ${s.label}`} onClick={() => handleSize(s.value)} className="text-sm px-2 py-1 hover:bg-accent rounded-lg text-left transition-colors flex items-center gap-2">
      ...
      {activePopover === "emoji" && renderPopover(
        <div className="p-3 bg-background border border-border rounded-lg shadow-xl animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="flex flex-wrap gap-1 mb-2">
            {EMOJI_TABS.map((tab) => (
              <button type="button" key={tab.key} onClick={() => setEmojiTab(tab.key)} title={tab.label} aria-label={tab.label} className={`h-8 w-8 flex items-center justify-center rounded-full border transition-colors ${tab.key === activeEmojiTab.key ? "border-primary text-primary bg-primary/10" : "border-border text-muted-foreground hover:text-foreground"}`}>
